...
1. **Programmed lift goals** — optional per-exercise auto-progression overlay
...
2. **Target-weight goal auto-picks calorie mode** — optional target weight on
...
3. **Tightened `ledger-api` CORS** — `ALLOWED_ORIGIN` set in Vercel's
   production env vars to `https://ledger-lovat-eight.vercel.app` (was `*`).
4. **Accounts Phase 2** — migrated every entity (profile, workouts, cardio,
   ...
   correctly.
5. **Accounts Phase 1** — Supabase email/password auth gate (`src/screens/Login.tsx`,
...
6. **Food API fixes** — `ledger-api` deployed as its own Vercel project;
   ...
   exceed that).
7. **Mobile UX fixes** — hardware/gesture back button now closes the current
...
8. **"Fits the philosophy" batch** — Metric/Imperial unit toggle, muscle-group
   ...
   arms/hips).
9. **"Quick wins" batch** (earlier) — Recent-foods shelf, edit-in-place for
