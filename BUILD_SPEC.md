...
- **Routines:** user-created and named, holding only an ordered exercise list — no weight/reps/sets on the routine itself; those are entered per exercise during a session. Push/Pull/Leg Day (§7) are seeded once on first run as ordinary, deletable/user-editable-equivalent routines, not a separate hardcoded tier.
  ...
  - **Start a routine → session:** a checklist of that routine's exercises. Tapping one opens the normal add flow pre-filled with that exercise name and its most recent session's sets (rest timer, overload hint, and plate calculator all still available) — the exercise/search picker is hidden here since it's already chosen; it only reappears for a fresh ad-hoc "Log exercise" or when editing a past entry, where switching to a different exercise entirely is a real use case. Saving returns to the checklist rather than to the idle Lift screen, so the user can work through the rest of the list. "Done" is derived from today's actual logged workouts (not a per-visit flag), so leaving and re-entering the same session still shows what's already been done today.
